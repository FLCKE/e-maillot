...
router.post('/', (req, res) => {
    ...
    hashPassword(password).then((result) => {
        if (result) {
            const query = 'INSERT INTO users (username, lastname, firstname, email, password, registrationDate,tokens, role) VALUES (?, ?, ?, ?, ?, ?, ?)';
            db.query(query, [username, lastname, firstname, email, result, registrationDate,tokens, role], (err, results) => {
                if (err) {
                    console.error('Error inserting user:', err);
                    res.status(500).send('Error inserting user');
                    return;
                }
                res.status(201).json({ userid: results.insertId, username, lastname, firstname, email, password, registrationDate,tokens, role });
            });
        }
    })
